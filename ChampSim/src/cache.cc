    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if(block[set][way].valid){
            ...
            if (cache_type == IS_LLC) {
                ...
                for (int cpu = 0; cpu < NUM_CPUS; cpu++) {
                    upper_level_dirty = ((CACHE *)upper_level_icache[cpu])->invalidate_and_return_data(cpu, block[set][way].address, &data, &data_cache);
                    
                    if (upper_level_dcache[cpu] != upper_level_icache[cpu]) {
                        upper_level_dirty = ((CACHE *)upper_level_dcache[cpu])->invalidate_and_return_data(cpu, block[set][way].address, &data, &data_cache);
                    }
        ...
        if (do_fill) {
            ...
            if (cache_type == IS_L1D)
                l1d_prefetcher_cache_fill(MSHR.entry[mshr_index].full_addr, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, block[set][way].full_addr, MSHR.entry[mshr_index].pf_metadata);
            if  (cache_type == IS_L2C)
                l2c_prefetcher_cache_fill(MSHR.entry[mshr_index].full_addr, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, block[set][way].full_addr, MSHR.entry[mshr_index].pf_metadata);
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (do_fill) {
                    // update prefetcher
                    if (cache_type == IS_L1D)
                        l1d_prefetcher_cache_fill(WQ.entry[index].full_addr, set, way, 0, block[set][way].full_addr, WQ.entry[index].pf_metadata);
                    else if (cache_type == IS_L2C)
                        l2c_prefetcher_cache_fill(WQ.entry[index].full_addr, set, way, 0, block[set][way].full_addr, WQ.entry[index].pf_metadata);
