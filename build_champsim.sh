...
if [ "$#" -ne 8 ]; then
    ...
    echo "Usage: ./build_champsim.sh [branch_pred] [l1i_pref] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [num_core] [policy]"
    exit 1
fi
...
if [ ! -f ./src/cache.${POLICY}cc]; then
    echo "[ERROR] Cannot find cache policy"
    echo "[ERROR] Possible cache policy from src/cache.*cc"
    find src -name "cache.*cc"
    exit 1
fi
...
cp src/cache.${POLICY}cc src/cache.cc
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core-${POLICY}"
...
cp src/cache.incc src/cache.cc